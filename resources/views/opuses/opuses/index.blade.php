            ...
            <div class="box col-md-12">
                <div class="box-inner">
                    <div class="box-header well" data-original-title="">
                        <h2><i class="glyphicon"></i>Opuses</h2>
                    </div>

                    <div>
                        <div style="text-align: left">
                            <form method="GET" action="{{ route('opuses.index') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right">
                                <p><select name="pages">
                        ...
                        <div style="text-align: right">
                            <form method="GET" action="{{ route('opuses.index') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                    ...
                    <div class="box-content">
                        <table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
                            ...
                            <tr>
                                ...
                                <th>Title <a href="#">
									...
									@endif
										</span></a></th><th>Text <a href="#">
									@if (request("sort") == "text")
										<span class="{{ request("toggle") }}" name="text">
									@else
										<span class="toggle glyphicon glyphicon-arrow-down" name="text">
									@endif
										</span></a></th>
                                <th>
                                    <a href="{{ route('opuses.create') }}" class="btn btn-success">
                                        <i class="glyphicon glyphicon-plus" aria-hidden="true"></i> Добавить
                                    </a>
									<form style="text-align: right" id="arrDelete" action="{{ route('opuses.arrDelete') }}" method="post">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
                            ...
                            <tbody>
                                @foreach($opuses as $item)
                                    <tr>
                                        {{-- <td>{{ $loop->iteration ?? $item->id }}</td> --}}
										<td><input class="arrDelete" type="checkbox" form="arrDelete" name="{{ $item->id }}" value="1"></td>
                                        <td>{{ $item->title }}</td><td>{!! $item->text !!}</td>
                                        <td>
                                            <a class="btn btn-success" href="{{ route('opuses.show', [$item->id]) }}"><i class="glyphicon glyphicon-zoom-in icon-white"></i></a>
                                            <a class="btn btn-info" href="{{ route('opuses.edit', [$item->id]) }}"><i class="glyphicon glyphicon-edit icon-white"></i></a>

                                            <form method="POST" action="{{ route('opuses.destroy', [$item->id]) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
						...
						<div>
							<span>
								Showing {{ $opuses->total() > 0 ? $opuses->currentPage() * $opuses->perPage() - $opuses->perPage() + 1 : 0 }} to {{ $opuses->currentPage() * $opuses->perPage() <= $opuses->total() ? $opuses->currentPage() * $opuses->perPage() : $opuses->total() }} of {{ $opuses->total() }} entries
							</span>
						</div>
                        <div class="pagination-wrapper" style="text-align: center"> {{ $opuses->appends(['pages' => request('pages'), 'search' => request('search'), 'toggle' => request('toggle'), 'sort' => request('sort')])->links() }} </div>
                    </div>
                </div>
            </div>
            <!--/span-->
